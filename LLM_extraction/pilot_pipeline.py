...
# it seems like sometimes we are missing the article title? 
# we will probably need different prompts for 

test = trace_comment_thread(comm_id[0], parent_id[0], collection = 'Breitbart', retrieve_article=True)



user_df = load_comments_from_jsonl_gz(user_name, base_dir)
comment_id = '1000981651'
timestamp = user_df[user_df['_id'] == '1000981651']['createdAt'].iloc[0]
mmyy = timestamp.strftime("%m%y")
...
guy = int(user_df['parent'][2])
client = MongoClient("mongodb://localhost:27017/")
comment_db = client["Comments"]
comment_collection = comment_db['Breitbart']
test = comment_collection.find_one({"_id": '1000896130'})
test
tst = comment_collection.find_one({"_id": '1000963028'})
tst['raw_message']
# get the other stuff 
# problem: user_df_top[0]
tct = trace_comment_thread(user_df_top[1], collection = 'Breitbart', verbose=True, retrieve_article=True)
...
df[df['id'] == sample_id]['topic'].iloc[0]